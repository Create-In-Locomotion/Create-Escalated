...
public class EscalatedShapes {
    ...
    public static final VoxelShaper
            ...
            NARROW_ESCALATOR_MIDDLE = VoxelShaper.forHorizontal(createMiddleEscalatorSlope(true, true), Direction.SOUTH),
    ...
    private static VoxelShape createMiddleEscalatorSlope(boolean left, boolean right) {
        VoxelShape shape = Shapes.empty();
        VoxelShape mainSlice = box(0, -14, 0, 16, -1, 1);
        VoxelShape leftSlice = box(0, -1, 0, 2, 8, 1);
        VoxelShape rightSlice = box(14, -1, 0, 16, 8, 1);
        for (int i = 0; i < 16; ++i) {
            float f = i / 16f;
    ...
    private static VoxelShape createTopEscalatorShape(boolean left, boolean right) {
        VoxelShape shape = Shapes.or(box(0, 0, 8, 16, 6, 16), box(0, 6, 8, 2, 16, 16), box(14, 6, 8, 16, 16, 16));
        VoxelShape mainSlice = box(0, -14, 0, 16, -1, 1);
        VoxelShape leftSlice = box(0, -1, 0, 2, 8, 1);
        VoxelShape rightSlice = box(14, -1, 0, 16, 8, 1);
        for (int i = 0; i < 9; ++i) {
        ...
        shape = Shapes.or(shape, box(0, 13, 15, 16, 13.5, 16));
        return shape;
    }
